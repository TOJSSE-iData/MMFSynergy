...
from models.datasets import TextDatasetForMLM
...
from models.models import BertWithoutSegEmbForMaskedLM
...
def get_dataloader(config: BaseConfig):
    def _get_dataloader(usage):
        data_set = TextDatasetForMLM(
            config.dataset[usage].files, tokenizer, special_tokens,
            tknz_cfg['vocab_size'], **config.mlm
        )
        loader = DataLoader(
            data_set, 
...
def main(config: BaseConfig):
    ...
    model = BertWithoutSegEmbForMaskedLM(model_cfg, model_cfg.add_pooler)
    model.to(device)
    logger.info(model)
